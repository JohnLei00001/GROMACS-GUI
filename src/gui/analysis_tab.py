...
class AnalysisTab(QWidget):
    ...
    def init_ui(self):
        ...
        # === 1. 轨迹处理 (trjconv) ===
        trj_card = StepCard(1, tr("轨迹处理 (去除周期性边界条件)"), layout_kind="form")
        trj_layout = trj_card.content_layout
        ...
        btn_trjconv = QPushButton(tr("运行 trjconv"))
        btn_trjconv.clicked.connect(self.run_trjconv)
        set_role(btn_trjconv, "primary")
        
        trj_layout.addRow(tr("输入轨迹 (-f):"), self.trj_input)
        trj_layout.addRow(tr("输入 TPR (-s):"), self.trj_tpr)
        trj_layout.addRow(tr("输出轨迹 (-o):"), self.trj_output)
        trj_layout.addRow(tr("PBC 处理 (-pbc):"), self.pbc_combo)
        trj_layout.addRow(tr("居中 (-center):"), self.center_check)
        trj_layout.addRow("", btn_trjconv)
        
        layout.addWidget(trj_card)
        
        # === 2. 数据分析 (RMSD, RMSF, Gyrate) ===
        ana_card = StepCard(2, tr("数据分析"), layout_kind="vbox")
        ana_layout = ana_card.content_layout
        
        # RMSD
        rmsd_layout = QHBoxLayout()
        btn_rmsd = QPushButton(tr("计算 RMSD (gmx rms)"))
        btn_rmsd.clicked.connect(self.run_rmsd)
        set_role(btn_rmsd, "primary")
        btn_plot_rmsd = QPushButton(tr("绘图 RMSD"))
        btn_plot_rmsd.clicked.connect(lambda: self.plot_xvg("rmsd.xvg", "RMSD", "Time (ps)", "RMSD (nm)"))
        set_role(btn_plot_rmsd, "primary")
        ...
        btn_rmsf = QPushButton(tr("计算 RMSF (gmx rmsf)"))
        btn_rmsf.clicked.connect(self.run_rmsf)
        set_role(btn_rmsf, "primary")
        btn_plot_rmsf = QPushButton(tr("绘图 RMSF"))
        btn_plot_rmsf.clicked.connect(lambda: self.plot_xvg("rmsf.xvg", "RMSF", "Residue", "RMSF (nm)"))
        set_role(btn_plot_rmsf, "primary")
        ...
        btn_gyrate = QPushButton(tr("计算回转半径 (gmx gyrate)"))
        btn_gyrate.clicked.connect(self.run_gyrate)
        set_role(btn_gyrate, "primary")
        btn_plot_gyrate = QPushButton(tr("绘图 Gyrate"))
        btn_plot_gyrate.clicked.connect(lambda: self.plot_xvg("gyrate.xvg", "Radius of Gyration", "Time (ps)", "Rg (nm)"))
        set_role(btn_plot_gyrate, "primary")
        ...
        # === 3. 可视化集成 ===
        vis_card = StepCard(3, tr("外部可视化工具"), layout_kind="vbox")
        vis_layout = QHBoxLayout()
        
        btn_vmd = QPushButton(tr("尝试启动 VMD"))
        btn_vmd.clicked.connect(self.launch_vmd)
        set_role(btn_vmd, "primary")
        
        btn_pymol = QPushButton(tr("尝试启动 PyMOL"))
        btn_pymol.clicked.connect(self.launch_pymol)
        set_role(btn_pymol, "primary")
        
        vis_layout.addWidget(btn_vmd)
        vis_layout.addWidget(btn_pymol)
        
        vis_card.content_layout.addLayout(vis_layout)
        layout.addWidget(vis_card)

        if not HAS_MATPLOTLIB:
            warn_label = QLabel(tr("提示: 未检测到 matplotlib，绘图功能不可用。请运行 pip install matplotlib"))
            set_role(warn_label, "error")
    ...
    def _get_group_map(self, cwd):
        ...
        if not tpr_file:
            self.main_window.log(tr("[分析] 未找到 .tpr 文件，无法自动检测索引组，将使用默认编号。"))
            self._group_cache[cwd] = {}
            return {}

        self.main_window.log(trf("[分析] 正在检测体系索引组 (基于 {file})...", file=tpr_file))
        ...
        if groups:
            self.main_window.log(trf("[分析] 检测到 {n} 个索引组。", n=len(groups)))
        else:
            self.main_window.log(tr("[分析] 未能解析索引组列表，将使用默认编号。"))
    ...
    def _resolve_group(self, cwd, preferred_names, fallback_number):
        ...
        for name in preferred_names:
            if name in groups:
                self.main_window.log(trf("[分析] 自动匹配组: '{name}' -> #{num}", name=name, num=groups[name]))
                return groups[name]
        self.main_window.log(trf("[分析] 未找到优选组 {names}，使用默认编号: #{num}", names=preferred_names, num=fallback_number))
    ...
    def run_trjconv(self):
        ...
        if not os.path.exists(os.path.join(cwd, trj_in)):
            QMessageBox.warning(self, tr("警告"), trf("未找到输入轨迹: {file}", file=trj_in))
            return
    ...
    def on_trj_finished(self, success, message):
        ...
        if success:
            QMessageBox.information(self, tr("成功"), trf("轨迹处理完成！输出文件: {file}", file=self.trj_output.text()))
        else:
            QMessageBox.critical(self, tr("错误"), trf("trjconv 失败: {msg}", msg=message))
    ...
    def _run_analysis(self, tool, output_file, group_specs, desc):
        ...
        if not os.path.exists(os.path.join(cwd, trj_file)):
             QMessageBox.warning(self, tr("警告"), trf("未找到轨迹文件 ({file})，请先运行模拟或轨迹处理！", file=trj_file))
             return
    ...
    def on_analysis_finished(self, success, message, desc, output_file):
        self.set_buttons_enabled(True)
        if success:
            QMessageBox.information(self, tr("成功"), trf("{desc} 完成！生成了 {file}", desc=desc, file=output_file))
        else:
            QMessageBox.critical(self, tr("错误"), trf("{desc} 失败: {msg}", desc=desc, msg=message))

    # --- 绘图逻辑 ---
    
    def plot_xvg(self, filename, title, xlabel, ylabel):
        if not HAS_MATPLOTLIB:
            QMessageBox.warning(self, tr("警告"), tr("未安装 matplotlib，无法绘图。"))
            return
        ...
        if not os.path.exists(filepath):
            QMessageBox.warning(self, tr("警告"), trf("未找到数据文件: {file}\n请先运行相应的分析命令。", file=filename))
            return
        ...
        try:
            ...
            QMessageBox.critical(self, tr("错误"), trf("读取文件失败: {err}", err=str(e)))
            return
            
        if not x:
            QMessageBox.warning(self, tr("警告"), tr("数据文件为空或格式无法解析。"))
            return
    ...
    def save_plot(self, fig, source_filename, cwd):
        ...
        save_path, _ = QFileDialog.getSaveFileName(
            ...
            tr("保存图片"),
            default_path,
            tr("PNG Files (*.png);;PDF Files (*.pdf);;SVG Files (*.svg)")
        )
        ...
        try:
            ...
            self.main_window.log(trf("已保存图像: {path}", path=save_path))
            QMessageBox.information(self, tr("成功"), trf("图像已保存到:\n{path}", path=save_path))
        except Exception as e:
            QMessageBox.critical(self, tr("错误"), trf("保存图像失败: {err}", err=str(e)))
    ...
    def _launch_vis_tool(self, tool_name):
        ...
        try:
            ...
            self.main_window.log(trf(">>> 正在启动 {tool}...", tool=tool_name))
        except FileNotFoundError:
             QMessageBox.warning(self, tr("警告"), trf("未找到 {tool} 命令。请确保它已安装并添加到系统 PATH 中。", tool=tool_name))
        except Exception as e:
             QMessageBox.critical(self, tr("错误"), trf("启动 {tool} 失败: {err}", tool=tool_name, err=str(e)))
