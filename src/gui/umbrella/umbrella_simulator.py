...
class UmbrellaSimulator(QTabWidget):
    ...
    def __init__(self, main_window):
        ...
        self.addTab(self.build_tab, tr("1. 体系构建"))

        self.eq_tab = EquilibrationTab(main_window)
        self.addTab(self.eq_tab, tr("2. 平衡 (EM+NVT+NPT)"))

        self.pull_tab = PullTab(main_window)
        self.addTab(self.pull_tab, tr("3. Pull 模拟"))

        self.window_tab = WindowTab(main_window)
        self.addTab(self.window_tab, tr("4. 窗口设置"))

        self.batch_tab = BatchTab(main_window)
        self.addTab(self.batch_tab, tr("5. 批量 MD"))

        self.wham_tab = WhamTab(main_window)
        self.addTab(self.wham_tab, tr("6. WHAM 分析"))
