...
class NvtTab(QWidget):
    ...
    def init_ui(self):
        ...
        self.status = QLabel(tr("等待 EM 完成..."))
        ...
        mdrun_card = StepCard(3, tr("执行 NVT 平衡 (mdrun)"))
    ...
    def _save_mdp(self):
        ...
        self.main_window.log(tr("已保存 nvt.mdp"))
    ...
    def _on_grompp_done(self, s, m):
        self._set_btns(True)
        if not s: QMessageBox.critical(self, tr("错误"), trf("grompp (NVT): {msg}", msg=m))
    ...
    def _on_mdrun_done(self, s, m):
        self._set_btns(True)
        if s:
            self.main_window.log(tr(">>> ✓ NVT 完成"))
            self.nvt_done.emit(self.cwd)
        else:
            QMessageBox.critical(self, tr("错误"), trf("mdrun (NVT): {msg}", msg=m))
