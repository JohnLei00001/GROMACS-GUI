...
class NvtTab(QWidget):
    ...
    def _run_grompp(self):
        ...
        for f in ["nvt.mdp", "em.gro", "topol.top"]:
            if not os.path.exists(os.path.join(self.cwd, f)):
                QMessageBox.warning(self, "警告", f"缺少 {f}，请先完成 EM"); return
    ...
    def _on_grompp_done(self, success, message):
        self._set_btns(True)
        if success:
            QMessageBox.information(self, "成功", "grompp 完成！已生成 nvt.tpr")
        else:
            QMessageBox.critical(self, "错误", f"grompp (NVT) 失败: {message}")

    def _run_mdrun(self):
        if not self.cwd: return
        if not os.path.exists(os.path.join(self.cwd, "nvt.tpr")):
            QMessageBox.warning(self, "警告", "未找到 nvt.tpr，请先运行 grompp"); return
    ...
    def _on_mdrun_done(self, success, message):
        ...
        if success:
            ...
            QMessageBox.information(self, "成功", "NVT 平衡完成！")
            self.nvt_done.emit(self.cwd)
        else:
            QMessageBox.critical(self, "错误", f"mdrun (NVT) 失败: {message}")
